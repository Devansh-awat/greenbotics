...
from src.obstacle_challenge.utils import FPSCounter
...
def initialize_all():
    ...
    if not vl53l0x.initialize(): return False
...
if __name__ == "__main__":
    ...
    future_initial_heading = INITIAL_HEADING
    frames_in_state = 0
    frames_since_block_lost = 0
    first_search_completed = False
    is_peek_avoidance = False
    is_dead_end_turn = False

    fps_counter = FPSCounter().start()
    alerted_for_low_fps = False # Flag to print alert only once
    ...
    try:
        while True:
            ...
            forward_dist = vl53l0x.get_distance(config.TOF_FORWARD_SENSOR_CHANNEL)

            # ===================================================
            # --- High-Priority Overrides ---
            # ===================================================
            # 1. Dead-End Override (Highest Priority)
            if forward_dist is not None and forward_dist < config.TOF_OBSTACLE_THRESHOLD_MM:
                # --- THIS IS THE FIX ---
                # Only trigger if we are on the main path (driving straight or approaching)
                if current_state in ["DRIVING_STRAIGHT"]:
                    print(f"\n[DEAD END OVERRIDE] Wall detected at {forward_dist} mm!")
                    is_dead_end_turn = True
                    current_state = "INITIATING_TURN"

            # 2. Block Proximity Override (Second Priority)
                ...
                if current_state not in ["INITIATING_TURN", "TURNING_WITH_GYRO", "CLEARING_OBSTACLE"]:
                    ...
                    is_peek_avoidance = current_state in ["SEARCHING_DRIVE", "APPROACHING_DURING_PEEK"]
                    current_state = "INITIATING_TURN"
            ...
            if current_state == "DRIVING_STRAIGHT":
                steer = steer_with_gyro(INITIAL_HEADING, current_yaw); motor.forward(config.DRIVE_SPEED)
                print(f"\rACTION: DRIVING STRAIGHT | Steer: {steer:5.1f}°", end="")
                if block_data: current_state = "APPROACHING_OBSTACLE"; print(f"\n[STATE CHANGE] ==> {current_state}")
                ...
                if not block_data: current_state = "DRIVING_STRAIGHT"; print(f"\n[STATE CHANGE] ==> {current_state}")

            elif current_state == "INITIATING_TURN":
                current_heading_at_decision = current_yaw if current_yaw is not None else INITIAL_HEADING
                
                if is_dead_end_turn:
                    turn_angle = config.DEAD_END_TURN_ANGLE if config.DEAD_END_TURN_DIRECTION == "RIGHT" else -config.DEAD_END_TURN_ANGLE
                    dynamic_target_heading = (current_heading_at_decision + turn_angle + 360) % 360
                    ...
                    turn_angle = config.AVOID_ANGLE_DEG if color == 'red' else -config.AVOID_ANGLE_DEG
                    dynamic_target_heading = (current_heading_at_decision + turn_angle + 360) % 360
                    print(f"\n[PEEK AVOIDANCE] Avoiding {color.upper()}. New Goal: {dynamic_target_heading:.1f}°")
                
                else: # Standard Lane Avoidance
                    color = block_data['color']
                    if color == 'red':
                        last_avoidance_direction = 'RIGHT'
                        dynamic_target_heading = (INITIAL_HEADING + config.AVOID_ANGLE_DEG + 360) % 360
                    else:
                        last_avoidance_direction = 'LEFT'
                        dynamic_target_heading = (INITIAL_HEADING - config.AVOID_ANGLE_DEG + 360) % 360
                    print(f"\n[LANE AVOIDANCE] Avoiding {color.upper()}. New Goal: {dynamic_target_heading:.1f}°")
                
                current_state = "TURNING_WITH_GYRO"; print(f"[STATE CHANGE] ==> {current_state}")

            elif current_state == "TURNING_WITH_GYRO":
                steer = steer_with_gyro(dynamic_target_heading, current_yaw); motor.forward(config.DRIVE_SPEED)
                ...
                if get_angular_difference(current_yaw, dynamic_target_heading) < config.HEADING_LOCK_TOLERANCE:
                    if is_dead_end_turn:
                        ...
                        current_state = "CLEARING_OBSTACLE"; frames_since_block_lost = 0
                    print(f"\n[STATE CHANGE] ==> {current_state}")
                    is_peek_avoidance = False; is_dead_end_turn = False
            
            elif current_state == "HEADING_RESET":
                INITIAL_HEADING = future_initial_heading
                dynamic_target_heading = INITIAL_HEADING
                first_search_completed = False; last_avoidance_direction = None
                print(f"\n[HEADING RESET] New 'straight' is {INITIAL_HEADING:.1f}°. Resetting mission.")
                current_state = "DRIVING_STRAIGHT"; print(f"[STATE CHANGE] ==> {current_state}")

            elif current_state == "CLEARING_OBSTACLE":
                steer = steer_with_gyro(dynamic_target_heading, current_yaw); motor.forward(config.DRIVE_SPEED)
                if not block_data: frames_since_block_lost += 1
                else: frames_since_block_lost = 0
                ...
                if frames_since_block_lost > config.CLEARANCE_FRAMES:
                    current_state = "RETURNING_TO_CENTER"; print(f"\n[STATE CHANGE] ==> {current_state}")
                ...
                if get_angular_difference(current_yaw, INITIAL_HEADING) < config.HEADING_LOCK_TOLERANCE:
                    current_state = "SCANNING_AT_CENTER"; frames_in_state = 0; print(f"\n[STATE CHANGE] ==> {current_state}")
                ...
                if frames_in_state > config.SCAN_DURATION_FRAMES:
                    if not first_search_completed:
                        current_state = "SEARCHING_PEEK_TURN"; print(f"\n[STATE CHANGE] ==> {current_state}")
                    else:
                        current_state = "DRIVING_STRAIGHT"; print(f"\n[STATE CHANGE] ==> {current_state}")
                ...
                dynamic_target_heading = peek_left if last_avoidance_direction == 'RIGHT' else peek_right
                steer = steer_with_gyro(dynamic_target_heading, current_yaw); motor.forward(config.DRIVE_SPEED)
                print(f"\rACTION: PEEK TURN | Steer: {steer:5.1f}° | Goal: {dynamic_target_heading:.1f}°", end="")
                if get_angular_difference(current_yaw, dynamic_target_heading) < config.HEADING_LOCK_TOLERANCE:
                    current_state = "SEARCHING_DRIVE"; frames_in_state = 0; print(f"\n[STATE CHANGE] ==> {current_state}")
                ...
                if block_data:
                    current_state = "APPROACHING_DURING_PEEK"; print(f"\n[STATE CHANGE] ==> {current_state}")
                elif frames_in_state > config.SEARCH_DRIVE_FRAMES:
                    first_search_completed = True; current_state = "RETURNING_TO_CENTER"; print(f"\n[STATE CHANGE] ==> {current_state} (Search Finished)")
                ...
                if not block_data:
                    frames_in_state = 0; current_state = "SEARCHING_DRIVE"; print(f"\n[STATE CHANGE] ==> {current_state}")
            
            fps_counter.update()
            fps_counter.display_on_frame(overlay_frame, alert_threshold=30.0)

            if fps_counter.fps < 30.0 and not alerted_for_low_fps and fps_counter._num_frames > 100:
                print(f"\n[WARNING] FPS has dropped to {fps_counter.fps:.1f}. Check for performance bottlenecks.")
